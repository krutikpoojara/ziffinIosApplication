...
class TodayMenuViewController: UIViewController,UITableViewDataSource,CustomIOS7AlertViewDelegate {
    ...
    override func viewDidLoad() {
        ...
        lblPrice.text = "Price : \(quantity) X \(tiffin_price)"
    ...
    func fetchCart_counter()
    {
        ...
        dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0)) { () -> Void in
            let json = JSON(data: NSData(contentsOfURL: NSURL(string: "http://shreehariconsultant.com/cooker_webservice/fatch_today_extra_cart.php?d_id=\(self.c_id)")!)!)
            print(json)
    ...
    func FetchData()
    {
        ...
        print(self.c_id)
        dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0)) { () -> Void in
            let json = JSON(data: NSData(contentsOfURL: NSURL(string: "http://shreehariconsultant.com/cooker_webservice/fatch_today_extra.php?d_id=\(self.c_id)")!)!)
            let tbl_extra_order_cart = json["tbl_extra_order_cart"]
            print(json)
            
            self.ce_id = tbl_extra_order_cart[0]["ed_id"].intValue
            for (index,_):(String, JSON) in tbl_extra_order_cart {
                
                let t_day = tbl_extra_order_cart[Int(index)!]["pro_name"].stringValue
                self.nameArr.append(t_day)
                print(t_day)
                let e_id = tbl_extra_order_cart[Int(index)!]["ed_id"].intValue
    ...
    @IBAction func btnCartAction(sender: AnyObject) {
        let secondViewController = self.storyboard?.instantiateViewControllerWithIdentifier("todayCart") as! TodayCartViewController
        ...
        alertView.onButtonTouchUpInside = { (alertView: CustomIOS7AlertView, buttonIndex: Int) -> Void in
            ...
            if buttonIndex == 1
            {
                ...
                else
                {
                    ...
                    let id = self.defaults.integerForKey("u_id")
                    print(id)
                    Alamofire.request(.GET, "http://shreehariconsultant.com/cooker_webservice/delete_today_tiffin.php", parameters: ["d_id":self.c_id,"u_id":id])
                        .responseJSON { response in
                            print(response.request)  // original URL request
                            print(response.response) // URL response
                            print(response.data)     // server data
